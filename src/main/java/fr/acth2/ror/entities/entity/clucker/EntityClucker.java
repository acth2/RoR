package fr.acth2.ror.entities.entity.clucker;
...
public class EntityClucker extends CluckerEntity implements IAnimatable {

    private final AnimationFactory factory = new AnimationFactory(this);
    private boolean stopEveryAnimations = false;

    public EntityClucker(EntityType<? extends CluckerEntity
            > type, World worldIn) {
        super(type, worldIn);
    }
    ...
    public void registerControllers(AnimationData data) {
        AnimationController<EntityClucker> controller = new AnimationController<>(this, "controller", 0, this::predicate);
        ...
        if (!isAttacking()) {
            if (event.isMoving()) {
                event.getController().setAnimation(
                        new AnimationBuilder().addAnimation("animation.clucker.walk", true)
                );
            } else {
                event.getController().setAnimation(
            ...
            event.getController().setAnimation(
                    new AnimationBuilder().addAnimation("animation.clucker.attack", false)
            );
            new Thread() {
                @Override
                public void run() {
